...
type UtterInput = {
  ...
  // apiKey removed - OpenAI no longer used
};
...
export async function POST(req: NextRequest) {
  ...
  // OpenAI removed - use rule-based fallback only
  // tryLLM and buildPrompt functions removed - no longer needed
  const llmText = null; // Always use fallback now
  console.log("[utter] Using rule-based fallback (OpenAI removed)");
  let outText = sanitize(ruleBasedFallback({ query, snippets, keywords, citations, stylePack }), mergedBans);
...
// tryLLM and buildPrompt functions removed - OpenAI no longer used
